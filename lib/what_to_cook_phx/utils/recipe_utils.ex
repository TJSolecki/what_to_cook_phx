defmodule WhatToCookPhx.Utils.RecipeUtils do
  @moduledoc """
  Utils for 
  """
  ...
  def get_recipe_from_url(url) do
    with {:ok, page} <- get_page(url),
      ...
      {:error, reason} ->
        Logger.error("get_recipe_from_url failed with reason: #{inspect(reason)}")
        {:error, reason}
    end
  end

  @spec parse_recipe_node(map(), String.t()) :: %Recipe{}
  defp parse_recipe_node(recipe_node, recipe_url) do
    %Recipe{
  ...
  defp parse_ingredients(recipe_node) do
  ...
  defp parse_image_url(recipe_node) do
  ...
  defp parse_instructions(recipe_node) do
  ...
  defp parse_yield(recipe_node) do
  ...
  defp get_recipe_node(ld_json_nodes) do
  ...
  defp type_is_recipe(type_value) do
